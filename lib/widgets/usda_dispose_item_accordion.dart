...
class UsdaDisposeItemAccordion extends StatefulWidget {
  ...
  const UsdaDisposeItemAccordion({
    super.key,
  ...
  @override
  State<UsdaDisposeItemAccordion> createState() =>
      _UsdaDisposeItemAccordionState();
}

class _UsdaDisposeItemAccordionState extends State<UsdaDisposeItemAccordion> {
  ...
  void initState() {
    ...
    if (status.startsWith('Dispose 1')) {
      _completedPathBeforeClose = 'left'; // Retailer path
    } else if (status == 'Dispose 2A: Disposed of Item') {
      _completedPathBeforeClose = 'right'; // Direct disposal path
    }
  }

  @override
  void didUpdateWidget(UsdaDisposeItemAccordion oldWidget) {
  ...
  bool _isStepCompleted(String stepStatus) {
    ...
    if (status == 'Completed' && widget.isLocked && _completedPathBeforeClose != null) {
      if (_completedPathBeforeClose == 'left') {
        // Retailer path - show both 1A and 1B as completed
        return stepStatus == 'Dispose 1A: Brought to local Retailer' ||
               stepStatus == 'Dispose 1B: Received Refund';
      } else if (_completedPathBeforeClose == 'right') {
        // Direct disposal path - show only 2A as completed
        return stepStatus == 'Dispose 2A: Disposed of Item';
      }
    }
  ...
  bool _isStepEnabled(int stepNumber) {
    ...
    // Initially, both step 1 (1A - Bring to Retailer) and step 3 (2A - Dispose) are enabled
    if (stepNumber == 1) {
      // Step 1A is enabled if no Dispose status OR if the current status is in the left path (1A or 1B)
      return !status.startsWith('Dispose') ||
          status.startsWith('Dispose 1');
    } else if (stepNumber == 2) {
      // Step 1B (Receive Refund) enabled only if step 1A is completed
      return status == 'Dispose 1A: Brought to local Retailer' ||
          status == 'Dispose 1B: Received Refund';
    } else if (stepNumber == 3) {
      // Step 2A (Dispose) is enabled if no left path chosen OR if it's the current 2A status (for toggling)
      return !status.startsWith('Dispose 1') ||
          ...
          if (widget.isExpanded && !widget.isDisabled)
            Container(
              ...
              child: Row(
                ...
                children: [
                  // Left Column - Retailer Path with white grouping box
                  Expanded(
                    child: Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          _buildStepButton(
                            ...
                            isCompleted: widget.recall.recallResolutionStatus.startsWith('Dispose 1'),
                          ...
                          _buildStepButton(
                            ...
                            status: 'Dispose 1B: Received Refund',
                            isCompleted:
                                _isStepCompleted('Dispose 1B: Received Refund'),
                  ...
                  // Right Column - Direct Disposal Path
                  Expanded(
                    child: Column(
                      children: [
                        // Step 2A Button
                        _buildStepButton(
                          ...
                          status: 'Dispose 2A: Disposed of Item',
                          isCompleted:
                              _isStepCompleted('Dispose 2A: Disposed of Item'),
