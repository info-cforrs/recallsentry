...
class AllVehicleRecallsPage extends StatefulWidget {
  const AllVehicleRecallsPage({super.key});

  @override
  State<AllVehicleRecallsPage> createState() => _AllVehicleRecallsPageState();
}

class _AllVehicleRecallsPageState extends State<AllVehicleRecallsPage> with HideOnScrollMixin {
  final RecallDataService _recallService = RecallDataService();
  final SubscriptionService _subscriptionService = SubscriptionService();
  final ArticleService _articleService = ArticleService();
  final TextEditingController _searchController = TextEditingController();
  final FocusNode _searchFocusNode = FocusNode();
  List<RecallData> _vehicleRecalls = [];
  ...
  bool _filterDoNotDrive = false;
  bool _filterFireRisk = false;
  ...
  Future<void> _loadVehicleRecalls() async {
    if (!mounted) return;
    ...
    try {
      ...
      final results = await Future.wait([
        _recallService.getNhtsaVehicleRecalls(limit: _pageSize, offset: 0),
      ...
      // Filter recalls by subscription tier cutoff date
  ...
  Future<void> _loadNextPage() async {
    ...
    try {
      ...
      final nextPageRecalls = await _recallService.getNhtsaVehicleRecalls(
        limit: _pageSize,
        offset: offset,
      );

      // Filter by subscription tier cutoff date
      ...
      setState(() {
        if (recentRecalls.isNotEmpty) {
          ...
          _vehicleRecalls.addAll(recentRecalls);
          _currentPage++;

          // Update filter options with new data
          _updateAvailableFilterOptions(_vehicleRecalls);
  ...
  void _applyFiltersAndSort() {
    List<RecallData> filtered = List.from(_vehicleRecalls);
    ...
    if (_searchQuery.isNotEmpty) {
      filtered = filtered.where((recall) {
        return recall.productName.toLowerCase().contains(
            ...
            recall.nhtsaVehicleMake.toLowerCase().contains(
              _searchQuery.toLowerCase(),
            ) ||
            recall.nhtsaVehicleModel.toLowerCase().contains(
              _searchQuery.toLowerCase(),
            ) ||
            recall.nhtsaComponent.toLowerCase().contains(
              _searchQuery.toLowerCase(),
            ) ||
    ...
    // Apply Do Not Drive filter
    if (_filterDoNotDrive) {
      filtered = filtered.where((recall) => recall.nhtsaDoNotDrive).toList();
    }

    // Apply Fire Risk filter
    if (_filterFireRisk) {
      filtered = filtered.where((recall) => recall.nhtsaFireRisk).toList();
    }
    ...
    switch (_sortOption) {
      case 'brand_az':
        filtered.sort(
          (a, b) {
            final aName = a.brandName.isNotEmpty ? a.brandName : a.nhtsaVehicleMake;
            final bName = b.brandName.isNotEmpty ? b.brandName : b.nhtsaVehicleMake;
            ...
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  ...
                  RadioListTile<String>(
                    title: const Text('Make (A-Z)', style: TextStyle(color: Colors.white)),
                    value: 'brand_az',
                    groupValue: _sortOption,
                    activeColor: const Color(0xFF64B5F6),
                    onChanged: (value) {
                  ...
                  RadioListTile<String>(
                    title: const Text('Make (Z-A)', style: TextStyle(color: Colors.white)),
                    value: 'brand_za',
                    groupValue: _sortOption,
                    activeColor: const Color(0xFF64B5F6),
                    onChanged: (value) {
  ...
  void _showFilterDialog() async {
    ...
    bool tempDoNotDrive = _filterDoNotDrive;
    bool tempFireRisk = _filterFireRisk;
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    CheckboxListTile(
                        ...
                        children: [
                          Container(
                            ...
                            child: const Row(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                Icon(Icons.warning_amber, color: Colors.yellow, size: 14),
                                SizedBox(width: 4),
                                Text(
                                  'DO NOT DRIVE',
                                  style: TextStyle(
                      ...
                      value: tempDoNotDrive,
                      activeColor: const Color(0xFF64B5F6),
                      onChanged: (value) {
                        setDialogState(() {
                          tempDoNotDrive = value ?? false;
                        });
                      },
                    ),
                    CheckboxListTile(
                ...
                ElevatedButton(
                  onPressed: () {
                    setState(() {
                      ...
                      _filterDoNotDrive = tempDoNotDrive;
                      _filterFireRisk = tempFireRisk;
                      _applyFiltersAndSort();
                    });
                    Navigator.of(context).pop();
                  },
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            _errorMessage.isNotEmpty
                ? _errorMessage
                : 'No vehicle recalls found in the last 30 days.',
            textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 16, color: Colors.white70),
          ),
          const SizedBox(height: 16),
          ElevatedButton(
            onPressed: _loadVehicleRecalls,
            style: ElevatedButton.styleFrom(
              backgroundColor: const Color(0xFF64B5F6),
            ),
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            if (!_hasMoreRecalls && _vehicleRecalls.isNotEmpty && !_isLoadingMore)
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 24.0),
                child: Text(
                  'All recalls loaded',
              ...
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Search vehicle recalls...',
