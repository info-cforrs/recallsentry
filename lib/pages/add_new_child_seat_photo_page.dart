...
import 'add_new_child_seat_details_page.dart';
...
class AddNewChildSeatPhotoPage extends StatefulWidget {
  ...
  @override
  State<AddNewChildSeatPhotoPage> createState() => _AddNewChildSeatPhotoPageState();
}

class _AddNewChildSeatPhotoPageState extends State<AddNewChildSeatPhotoPage>
  ...
  ChildSeatPhotoType? _selectedPhotoType;
  final Map<ChildSeatPhotoType, XFile?> _capturedPhotos = {};
  ...
  Future<void> _capturePhoto() async {
    ...
    try {
      ...
      // If seat label photo was captured, try to scan it for UPC and label info
      if (_selectedPhotoType == ChildSeatPhotoType.seatLabel) {
        await _scanLabelFromPhoto(photo);
      }
  ...
  Future<void> _scanLabelFromPhoto(XFile photo) async {
    setState(() {
      _isScanning = true;
    });

    try {
      // Try to scan UPC barcode from the photo
      final barcodeResult = await _barcodeService.scanFromXFile(photo);
      if (barcodeResult.found && barcodeResult.barcode != null) {
        setState(() {
          _scannedUpc = barcodeResult.normalizedUpc ?? barcodeResult.barcode;
        });
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('UPC detected: $_scannedUpc'),
              backgroundColor: Colors.green,
      ...
      // Try to perform OCR on the label
      final ocrResult = await _ocrService.scanLabel(photo);
      if (ocrResult.success && ocrResult.extractedFields.isNotEmpty) {
        setState(() {
          _labelScanResult = ocrResult;
        });
        if (mounted && _scannedUpc == null) {
          // Only show OCR message if no UPC was found
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Label text extracted'),
      ...
      // Scan failed silently - user can enter manually
  ...
  Future<void> _pickImageFromGallery() async {
    try {
      ...
      if (photo != null) {
        if (mounted) {
          final ChildSeatPhotoType? selectedType = await _showPhotoTypeDialog();
          if (selectedType != null) {
            ...
            // If seat label photo was selected, try to scan it
            if (selectedType == ChildSeatPhotoType.seatLabel) {
              await _scanLabelFromPhoto(photo);
            }
  ...
  Future<ChildSeatPhotoType?> _showPhotoTypeDialog() async {
    return showDialog<ChildSeatPhotoType>(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: ChildSeatPhotoType.values.map((type) {
              return ListTile(
                title: Text(
  ...
  Future<void> _proceedToNextPage() async {
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => AddNewChildSeatDetailsPage(
          photos: _capturedPhotos,
          scannedUpc: _scannedUpc,
          labelScanResult: _labelScanResult,
  ...
  String _getPhotoTypeLabel(ChildSeatPhotoType type) {
    switch (type) {
      case ChildSeatPhotoType.seatLabel:
        return 'SEAT LABEL';
      case ChildSeatPhotoType.frontView:
        return 'FRONT VIEW';
      case ChildSeatPhotoType.sideView:
        return 'SIDE VIEW';
      case ChildSeatPhotoType.backView:
        return 'BACK VIEW';
    }
  }
  ...
  Widget _buildPhotoSlot(ChildSeatPhotoType type) {
  ...
  IconData _getIconData(ChildSeatPhotoType type) {
    switch (type) {
      case ChildSeatPhotoType.seatLabel:
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                                    ...
                                    children: [
                                      Expanded(
                                          child: _buildPhotoSlot(
                                              ChildSeatPhotoType.seatLabel)),
                                      const SizedBox(height: 8),
                                      Expanded(
                                          child: _buildPhotoSlot(
                                              ChildSeatPhotoType.sideView)),
                                    ],
                                  ),
                                ),
                                ...
                                Expanded(
                                  child: Column(
                                    children: [
                                      Expanded(
                                          child: _buildPhotoSlot(
                                              ChildSeatPhotoType.frontView)),
                                      const SizedBox(height: 8),
                                      Expanded(
                                          child: _buildPhotoSlot(
                                              ChildSeatPhotoType.backView)),
                                    ],
                                  ),
                                ),
                              ],
                          ...
                          if (_scannedUpc != null || _labelScanResult != null) ...[
                            ...
                            Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (_labelScanResult != null && _labelScanResult!.extractedFields.isNotEmpty) ...[
                                    if (_scannedUpc != null) const SizedBox(height: 4),
                                    Row(
                                      children: [
                                        const Icon(Icons.document_scanner, color: Colors.green, size: 18),
                                        const SizedBox(width: 8),
                                        Text(
                                          '${_labelScanResult!.extractedFields.length} field(s) extracted from label',
                                          style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w500),
                                        ),
                                      ],
                                    ),
