...
class AllChildSeatRecallsPage extends StatefulWidget {
  const AllChildSeatRecallsPage({super.key});

  @override
  State<AllChildSeatRecallsPage> createState() => _AllChildSeatRecallsPageState();
}

class _AllChildSeatRecallsPageState extends State<AllChildSeatRecallsPage> with HideOnScrollMixin {
  final RecallDataService _recallService = RecallDataService();
  final SubscriptionService _subscriptionService = SubscriptionService();
  final ArticleService _articleService = ArticleService();
  final TextEditingController _searchController = TextEditingController();
  final FocusNode _searchFocusNode = FocusNode();
  List<RecallData> _childSeatRecalls = [];
  ...
  Future<void> _loadChildSeatRecalls() async {
    if (!mounted) return;
    ...
    try {
      ...
      final results = await Future.wait([
        _recallService.getNhtsaChildSeatRecalls(limit: _pageSize, offset: 0),
      ...
      setState(() {
        if (recentRecalls.isNotEmpty) {
          _childSeatRecalls = recentRecalls;
  ...
  Future<void> _loadNextPage() async {
    ...
    try {
      ...
      final nextPageRecalls = await _recallService.getNhtsaChildSeatRecalls(
        limit: _pageSize,
        offset: offset,
      );
      ...
      setState(() {
        if (recentRecalls.isNotEmpty) {
          _childSeatRecalls.addAll(recentRecalls);
          _currentPage++;
          _updateAvailableFilterOptions(_childSeatRecalls);
          _applyFiltersAndSort();
          _hasMoreRecalls = recentRecalls.length == _pageSize;
        } else {
          _hasMoreRecalls = false;
  ...
  void _applyFiltersAndSort() {
    List<RecallData> filtered = List.from(_childSeatRecalls);
    ...
    if (_searchQuery.isNotEmpty) {
      filtered = filtered.where((recall) {
        return recall.productName.toLowerCase().contains(_searchQuery.toLowerCase()) ||
            ...
            recall.nhtsaUpc.toLowerCase().contains(_searchQuery.toLowerCase()) ||
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(
            _errorMessage.isNotEmpty ? Icons.error_outline : Icons.child_care,
            size: 80,
            color: _errorMessage.isNotEmpty ? Colors.red : Colors.white54,
          ),
          const SizedBox(height: 16),
          Text(
            _errorMessage.isNotEmpty ? 'Error Loading Recalls' : 'No Child Seat Recalls Found',
            style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white),
          ),
          const SizedBox(height: 8),
          Text(
            _errorMessage.isNotEmpty ? _errorMessage : 'No child seat recalls found in the last 30 days.',
            textAlign: TextAlign.center,
            style: const TextStyle(fontSize: 16, color: Colors.white70),
          ),
          const SizedBox(height: 16),
          ElevatedButton(
            onPressed: _loadChildSeatRecalls,
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            if (!_hasMoreRecalls && _childSeatRecalls.isNotEmpty && !_isLoadingMore)
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 24.0),
              ...
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Search car seat recalls...',
